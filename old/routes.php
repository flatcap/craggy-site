...
function mark_panel ($row, &$old_panel)
{
    $p = $row['panel'];
...
function mark_age ($row, &$old_age)
{
    $a = strtotime ($row['date_set']);

    $result = (abs($a - $old_age) > 7);
    if ($result) {
        $old_age = $a;
    }
...
function mark_grade ($row, &$old_grade)
{
    $g = $row['grade'];
...
function mark_setter ($row, &$old_setter)
{
    $s = $row['setter'];
